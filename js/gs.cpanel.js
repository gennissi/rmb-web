				...
				if (response)
				{
					...
					$.ajax({
						...
						success: function(result)
						{
							...
							{
								document.getElementById('send_email_status').innerHTML = la['CANT_SEND_EMAIL'] + ' ' + la['PLEASE_CHECK_SERVER_EMAIL_SMTP_SETTINGS'];
							}
						}
					});
				}
...
function notifyCheck(what)
{
	switch (what)
	{
		...
		case "smtp_server":
			if (document.getElementById('cpanel_manage_server_email').value  == '')
			{
				if (cpValues['privileges'] == 'super_admin')
				{
					confirmDialog(la['PLEASE_SET_SMTP_SERVER_SETTINGS_IN_MANAGE_SERVER_SECTION'], function(response){
						if (response)
						{
							switchCPTab('manage_server');
							document.getElementById('manage_server_email_tab').click();
						}
					});	
				}				
			}
