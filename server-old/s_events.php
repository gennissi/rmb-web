...
function event_notify($ed,$ud,$od,$loc)
{
    ...
    {
        $q1 = "SELECT * FROM `gs_fcm` WHERE `username`='".$ud['username']."' OR `email`='".$ud['email']."'";
        $r1 = mysqli_query($ms, $q1);

        $template = mysqli_real_escape_string($ms, $od['name']).' : '.mysqli_real_escape_string($ms, $ed['event_desc']).' , '.$loc['dt_server'];

        while ($row1 = mysqli_fetch_array($r1))
        {
            sendPushQueue($row1['token'], 'event', $template);
        }
    }
