	...
	if(@$_POST['cmd'] == 'load_billing_plan_purchase_list')
	{
		...
		while($row=mysqli_fetch_array($r))
		{
			...
			if ($active == 'true')
			{
				...
				// generate url
				if ($gsValues['BILLING_GATEWAY'] == 'paypalv2')
				{
					$custom = $gsValues['BILLING_PAYPAL_CUSTOM'].','.$_SESSION["email"].','.$plan_id;
				
					$payment_url = $gsValues['URL_ROOT'].'/mod/paypal/index.php?currency='.$gsValues['BILLING_CURRENCY'].'&price='.$price.'&name='.$name.' '.$_SESSION["email"].'&custom='.$custom;		
				}
				else if ($gsValues['BILLING_GATEWAY'] == 'paypal')
				{
					$custom = $gsValues['BILLING_PAYPAL_CUSTOM'].','.$_SESSION["email"].','.$plan_id;
