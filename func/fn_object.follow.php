...
<html>
    <head>
		...
		function showExtraData(imei, data)
		{
			...
			var list_str = 'side_panel_follow_data_list_grid';		
		...
		function initGrids()
		{
			...
			$(window).bind('resize', function()
			{
				if ($(window).width()< 640)
				{
					$("#side_panel_follow_data_list_grid").setGridHeight($(window).height() - 112);
				}
				else
				{
					$("#side_panel_follow_data_list_grid").setGridHeight($(window).height() - 37);
				}
			}).trigger('resize');
		}
