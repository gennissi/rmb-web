<? 
	...
	if(@$_POST['format'] == 'plc')
	{
		...
		$q = "SELECT * FROM `gs_user_places_groups` WHERE `user_id`='".$user_id."' ORDER BY `group_id` ASC";
		$r = mysqli_query($ms, $q);
		...
		{                        
			...
			{
				$group_id = 0;								
				if (isset($data['markers'][$i]['group_name']))
				{
					if (isset($groups[$data['markers'][$i]['group_name']]))
					{
						$group_id = mysqli_real_escape_string($ms, $groups[$data['markers'][$i]['group_name']]);
					}	
				}
				
				$marker_radius = 0.1;
				if (isset($data['markers'][$i]['radius']))
				{
					$marker_radius = mysqli_real_escape_string($ms, $data['markers'][$i]['radius']);
				}
		...
		{  
			...
			{
				$group_id = 0;								
				if (isset($data['routes'][$i]['group_name']))
				{
					if (isset($groups[$data['routes'][$i]['group_name']]))
					{
						$group_id = mysqli_real_escape_string($ms, $groups[$data['routes'][$i]['group_name']]);
					}	
				}
		...
		{  
			...
			{
				$group_id = 0;								
				if (isset($data['zones'][$i]['group_name']))
				{
					if (isset($groups[$data['zones'][$i]['group_name']]))
					{
						$group_id = mysqli_real_escape_string($ms, $groups[$data['zones'][$i]['group_name']]);
					}	
				}
